...
# Use the main run.py launcher (most reliable)
py_script = PROJECT_ROOT / "run.py"
if not py_script.exists():
    # Fallback to src files if run.py doesn't exist
    launcher_pyw = PROJECT_ROOT / "src" / "mediaslayer_launcher.pyw"
    if launcher_pyw.exists():
        py_script = launcher_pyw
    else:
        py_script = PROJECT_ROOT / "src" / "media_downloader_gui.py"
...
def create_shortcut():
    ...
    # --- Icon handling (assets folder resides at project root) ---
    png_icon = PROJECT_ROOT / "assets" / "icone.png"  # User-provided PNG
    ico_icon = PROJECT_ROOT / "assets" / "icone.ico"
    ...
    if png_icon.exists():
        ...
        # Fallback to existing mediaslayer.ico (now in assets/)
        default_icon = PROJECT_ROOT / "assets" / "mediaslayer.ico"
