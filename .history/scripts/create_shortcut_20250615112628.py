...
py_script = SCRIPT_DIR / "run.py"
if not py_script.exists():
    # Fallback to src files if run.py doesn't exist
    launcher_pyw = SCRIPT_DIR / "src" / "mediaslayer_launcher.pyw"
    if launcher_pyw.exists():
        py_script = launcher_pyw
    else:
        py_script = SCRIPT_DIR / "src" / "media_downloader_gui.py"
...
def create_shortcut():
    ...
    # --- Icon handling (now in assets/ folder) ---
    png_icon = SCRIPT_DIR / "assets" / "icone.png"  # User-provided PNG
    ico_icon = SCRIPT_DIR / "assets" / "icone.ico"
    ...
    if png_icon.exists():
        ...
        default_icon = SCRIPT_DIR / "assets" / "mediaslayer.ico"
