...
class MediaSlayerGUI:
    ...
    def create_url_section(self, parent):
        ...
        # Botão colar
        paste_btn = ttk.Button(url_frame, text="📋 Colar", 
                              command=self.paste_url, style='Quest.TButton')
        paste_btn.grid(row=1, column=1, padx=(0, 10))
        
        # Botão analisar
        analyze_btn = ttk.Button(url_frame, text="🔍 Analisar", 
                               command=self.analyze_url, style='Quest.TButton')
        analyze_btn.grid(row=1, column=2)
