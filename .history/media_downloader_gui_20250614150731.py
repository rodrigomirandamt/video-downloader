...
class MediaSlayerGUI:
    ...
    def create_main_card(self, parent):
        ...
        # Título do card
        card_title = ttk.Label(card_frame, text="🎯 Download de Mídia", 
                              font=('Segoe UI', 18, 'bold'), 
                              background='#252525', foreground='#ffffff')
        card_title.grid(row=0, column=0, pady=(0, 25))
    ...
    def create_url_section(self, parent):
        ...
        # Entry e botões
        entry_container = ttk.Frame(url_frame, style='Card.TFrame')
        entry_container.grid(row=1, column=0, sticky=(tk.W, tk.E))
        entry_container.columnconfigure(0, weight=1)
        
        self.url_entry = ttk.Entry(entry_container, textvariable=self.url_var, 
                                  style='Dark.TEntry', font=('Segoe UI', 14))
        self.url_entry.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=(0, 10), ipady=5)
        self.url_entry.bind('<KeyRelease>', self.on_url_change)
        
        # Botões de ação
        buttons_frame = ttk.Frame(entry_container, style='Card.TFrame')
        buttons_frame.grid(row=0, column=1)
        
        paste_btn = ttk.Button(buttons_frame, text="📋 Colar", 
                              command=self.paste_url, style='Quest.TButton')
        paste_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        analyze_btn = ttk.Button(buttons_frame, text="🔍 Analisar", 
                                command=self.analyze_url, style='Quest.TButton')
        ...
        self.platform_label.grid(row=2, column=0, pady=(10, 0))
    ...
    def create_settings_section(self, parent):
        ...
        quality_label = ttk.Label(quality_frame, text="👑 Qualidade:", 
                                 font=('Segoe UI', 12, 'bold'),
                                 background='#252525', foreground='#ffffff')
        ...
        self.quality_combo = ttk.Combobox(quality_frame, textvariable=self.quality_var,
                                         ...
                                         state="readonly", style='Dark.TCombobox', font=('Segoe UI', 11))
        ...
        self.format_combo = ttk.Combobox(format_frame, textvariable=self.format_var,
                                        values=["mp4 (Vídeo)", "mp3 (Áudio)", "webm (Web)"],
                                        state="readonly", style='Dark.TCombobox', font=('Segoe UI', 11))
        ...
        path_label = ttk.Label(path_frame, text="📂 Pasta de Destino:", 
                              font=('Segoe UI', 12, 'bold'),
                              background='#252525', foreground='#ffffff')
        path_label.grid(row=0, column=0, columnspan=2, sticky=tk.W, pady=(0, 8))
        
        self.path_entry = ttk.Entry(path_frame, textvariable=self.path_var, 
                                   style='Dark.TEntry', state='readonly', font=('Segoe UI', 11))
        self.path_entry.grid(row=1, column=0, sticky=(tk.W, tk.E), padx=(0, 10), ipady=5)
    ...
    def create_progress_section(self, parent):
        ...
        self.progress_bar = ttk.Progressbar(progress_frame, variable=self.progress_var,
                                           maximum=100, length=500, style='Quest.Horizontal.TProgressbar')
        self.progress_bar.grid(row=1, column=0, sticky=(tk.W, tk.E), ipady=3)
    
    def create_download_button(self, parent):
        """Criar botão de download"""
        self.download_btn = ttk.Button(parent, text="⬇️ Baixar Mídia", 
                                      command=self.start_download, style='Quest.TButton')
        self.download_btn.grid(row=4, column=0, pady=(15, 0), ipady=8)
    ...
    def progress_hook(self, d):
        ...
        if d['status'] == 'downloading':
            ...
            self.update_status("✅ Download concluído! Mídia salva com sucesso!")
    ...
    def download_media(self):
        ...
        try:
            ...
            messagebox.showinfo("Sucesso", "✅ Download concluído! Mídia salva com sucesso!")
            self.update_status("Pronto para próximo download")
            
        except Exception as e:
            messagebox.showerror("Erro", f"❌ Download falhou: {str(e)}")
            self.update_status("Download falhou - tente novamente")
        finally:
            self.progress_var.set(0)
            self.download_btn.config(state='normal', text="⬇️ Baixar Mídia")
